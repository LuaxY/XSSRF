$(function () {
	...
	uploadBtn.on('change', function (e) {
		...
		if (files.size > MAX_SIZE) {
			...
			$('.file-name', totalRow).text('your upload is too big')
			return
		}

		var up = files.upload("upload")
			...
			switch (e.target.status) {
				case 200:
					...
					eachRow(res.files, function (row, file, files) {
						var link = $('<a>')
						link.attr('href', 'https://upimgs.net/' + file.url)
							.attr('target', '_BLANK')
							.text('upimgs.net/' + file.url)
				...
				case 413:
					...
					totalName.html($('<div/>').html('your upload is too big').text());
					break
				default:
					uploadFiles.addClass('error completed')
